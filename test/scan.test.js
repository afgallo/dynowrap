...
describe('Scan', () => {
  ...
  describe('#exec', () => {
    it('should call run scan on table', (done) => {
      table.runScan.yields(null, {
        ConsumedCapacity: { CapacityUnits: 5, TableName: 'accounts' },
        Count: 10,
        ScannedCount: 12
      })
  ...
  describe('#where', () => {
    ...
    it('should have multiple filters', () => {
      scan = scan.where('name').equals('Tim').where('email').beginsWith('foo')
    ...
    it('should have multiple filters on the same attribute', () => {
      scan = scan.where('email').gt('foo@example.com').where('email').lt('moo@example.com')
