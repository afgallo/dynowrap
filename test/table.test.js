...
describe('table', () => {
  ...
  describe('#destroy', () => {
    ...
    it('should parse and return attributes', (done) => {
      ...
      serializer.buildKey.returns(request.Key)
      serializer.deserializeItem.withArgs(returnedAttributes).returns({ email: 'test@test.com', name: 'Foo Bar' })
  ...
  describe('#dynamoCreateTableParams', () => {
    ...
    it('should make table arguments with secondary index', () => {
      const config = {
        hashKey: 'name',
        rangeKey: 'email',
        indexes: [{ hashKey: 'name', rangeKey: 'age', name: 'ageIndex', type: 'local' }],
        schema: {
          name: Joi.string(),
          email: Joi.string(),
          age: Joi.number()
