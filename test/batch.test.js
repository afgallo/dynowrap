  ...
  describe('#getItems', () => {
    it('should get items by hash key', (done) => {
      ...
      const expectedRequest = {
        RequestItems: {
          accounts: {
            Keys: [{ email: 'test@test.com' }, { email: 'foo@example.com' }]
          }
        }
      }
    ...
    it('should get items by hash key with consistent read', (done) => {
      ...
      batch(table, Serializer).getItems(
        ['test@test.com', 'foo@example.com'],
        { ConsistentRead: true },
        (err, items) => {
          items.should.have.length(2)
          items[0].get('email').should.equal('test@test.com')
    ...
    it('should get items when encounters retryable excpetion', (done) => {
      ...
      table.runBatchGetItems.onCall(0).yields(err).onCall(1).yields(null, response)
    ...
    it('should get unprocessed keys', (done) => {
      ...
      const firstResponse = {
        Responses: {
          accounts: [{ email: 'test@test.com', name: 'Tim Tester' }]
        },
        UnprocessedKeys: {
          accounts: {
            Keys: [{ email: 'foo@example.com' }]
          }
        }
      }
      ...
      const secondResponse = {
        Responses: {
          accounts: [{ email: 'foo@example.com', name: 'Foo Bar' }]
        }
      }
      ...
      table.runBatchGetItems
        .withArgs(expectedFirstRequest)
        .yields(null, firstResponse)
        .withArgs(expectedSecondRequest)
        .yields(null, secondResponse)
