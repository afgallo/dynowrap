      ...
      (callback) => {
        async.times(
          ...
          (n, next) => {
            ...
            const actors = [{ firstName: 'Tom', lastName: 'Hanks', titles: ['Producer', 'Actor', 'Soundtrack'] }]
            ...
            Movie.create({ title: `Movie ${n}`, releaseYear: 2001 + n, actors, director, tags }, next)
          },
          callback
        )
      }
...
describe('dynowrap Integration Tests', function () {
  this.timeout(0)

  before((done) => {
    dynowrap.dynamoDriver(helper.realDynamoDB())

    User = dynowrap.define('dynowrap-int-test-user', {
      hashKey: 'id',
      schema: {
        ...
        roles: dynowrap.types.stringSet().default(['user']),
    ...
    Tweet = dynowrap.define('dynowrap-int-test-tweet', {
      hashKey: 'UserId',
      rangeKey: 'TweetID',
      schema: {
        UserId: Joi.string(),
      ...
      indexes: [{ hashKey: 'UserId', rangeKey: 'PublishedDateTime', type: 'local', name: 'PublishedDateTimeIndex' }]
    })

    Movie = dynowrap.define('dynowrap-int-test-movie', {
      hashKey: 'title',
      timestamps: true,
      schema: {
        title: Joi.string(),
    ...
    DynamicKeyModel = dynowrap.define('dynowrap-int-test-dyn-key', {
      hashKey: 'id',
      schema: Joi.object()
        .keys({
          id: Joi.string()
    ...
    async.series(
      [
        async.apply(dynowrap.createTables.bind(dynowrap)),
        (callback) => {
          const items = [{ fiz: 3, buz: 5, fizbuz: 35 }]
          User.create(
            { id: '123456789', email: 'some@user.com', age: 30, settings: { nickname: 'thedude' }, things: items },
            callback
          )
        },
        (callback) => {
  ...
  describe('#query', () => {
    ...
    it('should return movie if directed by Steven Spielberg the 4', (done) => {
      Movie.query('Movie 4')
        .filter('director.firstName')
        .equals('Steven')
        .filter('director.lastName')
        .equals('Spielberg the 4')
        .limit(2)
        .loadAll()
        .exec((err, data) => {
          expect(err).to.not.exist
  ...
  describe('timestamps', () => {
    ...
    before((done) => {
      Model = dynowrap.define('dynowrap-int-test-timestamp', {
        hashKey: 'id',
        timestamps: true,
        schema: {
          id: Joi.string()
      ...
      ModelCustomTimestamps = dynowrap.define('dynowrap-int-test-timestamp-custom', {
        hashKey: 'id',
        timestamps: true,
        createdAt: 'created',
        updatedAt: 'updated',
      ...
      return dynowrap.createTables(done)
    })
