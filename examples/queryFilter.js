...
const Account = dynowrap.define('example-query-filter', {
  hashKey: 'name',
  rangeKey: 'email',
  timestamps: true,
  schema: {
  ...
  indexes: [{ hashKey: 'name', rangeKey: 'createdAt', type: 'local', name: 'CreatedAtIndex' }]
})
...
const loadSeedData = (callback) => {
  ...
  async.times(
    ...
    (n, next) => {
      ...
      Account.create({ email: `test${n}@example.com`, name: `Test ${n % 3}`, age: n, roles }, next)
    },
    callback
  )
}
...
async.series([async.apply(dynowrap.createTables.bind(dynowrap)), loadSeedData], (err) => {
  if (err) {
